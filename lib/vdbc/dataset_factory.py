        ...
        if os.path.exists(self.__fimg) and os.path.exists(self.__fgt) and not flush:
            ...
            for folder in video_list:
                ...
                self.__gt_info[folder] = [(float(box[0]), float(box[1]), float(box[2]), float(box[3]))
                                          for box in gt_list]
                if len(self.__gt_info[folder]) < len(self.__image_list[folder]):
                    num_gt = len(self.__gt_info[folder])
                    num_im = len(self.__image_list[folder])
                    self.__gt_info[folder].extend([self.__gt_info[folder][-1]]*(num_im-num_gt))
